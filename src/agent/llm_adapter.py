...
class LLMToolAdapter:
    ...
    # ============================================================
    # Streaming completion (tool-call aware)
    # ============================================================

    def stream_call_with_tools(
        self,
        ...
        """Generator yielding (chunk_text, chunk_usage, error_msg) tuples from a streamed LLM call.

        For tool-calling responses, the LLM first emits the tool call declarations (non-streamed),
        then streams the text content. This method yields incremental text chunks as they arrive.

        Yields:
            (text_delta: str, usage: dict | None, error: str | None)
        """
        config = self._config
        ...
        for model in models_to_try:
            try:
                yield from self._stream_litellm_model(
                    messages,
                    tools or [],
                    model,
                    timeout=timeout,
                ...
                logger.warning("Agent LLM stream rate-limited on %s: %s", model, e)
                last_error = e
                continue
            except litellm.ContextWindowExceededError as e:
                logger.warning("Agent LLM stream context exceeded on %s: %s", model, e)
                last_error = e
                continue
            except Exception as e:
                logger.warning("Agent LLM stream failed on %s: %s", model, e)
                last_error = e
                continue

        error_msg = f"All LLM models failed for stream. Last error: {last_error}"
        logger.error(error_msg)
        yield "", None, error_msg

    def _stream_litellm_model(
        self,
        messages: List[Dict[str, Any]],
        tools: List[dict],
        model: str,
        *,
        ...
        """Call a specific litellm model with stream=True and yield text deltas."""
        ...
        for chunk in response:
            ...
            yield delta_text, chunk_usage, None

        # Signal completion with provider info
        yield "", {"provider": provider_name, "model": model}, None
    ...
    def _extract_stream_text(chunk: Any) -> str:
        ...
        if delta is not None:
            ...
            if content is None and isinstance(getattr(delta, "content_raw", None), str):
                content = delta.content_raw
