...
const fieldTitleMap: Record<string, string> = {
  ...
  DEBATE_BACKEND: '辩论后端',
  DEBATE_FALLBACK_BACKEND: '辩论回退后端',
  CAMEL_MODEL_PLATFORM: 'CAMEL 模型平台',
  CAMEL_MODEL_NAME: 'CAMEL 模型名称',
  CAMEL_BASE_URL: 'CAMEL API 地址',
  CAMEL_API_KEY: 'CAMEL API 密钥',
  CAMEL_TEMPERATURE: 'CAMEL 温度',
  CAMEL_MAX_TOKENS: 'CAMEL 最大 Token',
...
const fieldDescriptionMap: Record<string, string> = {
  ...
  AGENT_ARCH: "选择 Agent 执行架构。single 为经典单 Agent；multi 为内部多 Agent 编排；debate 为 CAMEL 多空辩论编排。",
  DEBATE_BACKEND: 'debate 模式使用的后端。camel 为主实现，internal 为原 DebateArena。',
  DEBATE_FALLBACK_BACKEND: 'CAMEL 初始化或执行失败时使用的回退后端。',
  CAMEL_MODEL_PLATFORM: 'CAMEL 模型平台。当前首版支持 OpenAI 兼容接口。',
  CAMEL_MODEL_NAME: '传递给 CAMEL 的模型名称。',
  CAMEL_BASE_URL: 'CAMEL 使用的 OpenAI 兼容 API 地址。',
  CAMEL_API_KEY: 'CAMEL 使用的 OpenAI 兼容接口密钥。',
  CAMEL_TEMPERATURE: 'CAMEL 角色调用的采样温度。',
  CAMEL_MAX_TOKENS: '每个 CAMEL 角色调用的最大输出 Token 数。',
