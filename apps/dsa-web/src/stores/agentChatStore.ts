  ...
  startStream: async (payload, meta) => {
    ...
    try {
      ...
      const processLine = (line: string) => {
        ...
        // Handle streaming content chunks from the generating event
        if (event.type === 'generating' && event.content) {
          if (!assistantMessageId) {
            ...
            // Append streaming content to existing assistant message
            set((s) => ({
      ...
      // If content was streamed incrementally, it's already in the messages array.
      // Only append a fallback message if nothing was streamed.
      if (shouldAppend && !assistantMessageId) {
        set((s) => ({
          messages: [
            ...s.messages,
            {
      ...
      } else if (shouldAppend && assistantMessageId && finalContent !== null) {
        // Streaming was active but done event may have additional content;
        // ensure the streamed message ends with the complete final content
        // if the done event provided one.
        set((s) => ({
          messages: s.messages.map((m) =>
            m.id === assistantMessageId && finalContent
